...
class EngineClient(object):
    ...
    def open_gui(self, panel="settings"):
        return self._request("POST", "/api/gui/open", payload={"panel": panel})

    def rescan(self):
        return self._request("POST", "/api/engine/rescan")

    def open_gui_with_fallback(self):
        primary = self.open_gui(panel="settings")
        if primary.get("ok"):
            return primary

        fallback = self.rescan()
    ...
    def enqueue_publish(self, file_path, meta=None, auto_process=True, **context_kwargs):
        ...
        return self._request("POST", "/api/publish/enqueue", payload=payload)
