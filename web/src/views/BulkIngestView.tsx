...
export function BulkIngestView({ onNavigateToQueue }: BulkIngestViewProps) {
  ...
  const requireEngine = (): boolean => {
    if (engineHealth.isChecking) {
      toast.info("Checking local engine connection…")
      return false
    }
    if (!engineHealth.isOnline) {
      toast.error(
        engineHealth.offlineHelpText ??
          `CTrack Engine is not reachable at ${engineHealth.engineBase}. Start the tray on this PC.`,
      )
      return false
    }
    return true
  ...
  const handleBrowse = async (): Promise<void> => {
    ...
    try {
      ...
      if (!ipcRenderer) {
        toast.error("Engine bridge is not loaded. Reload the page.")
        return
      }
      ...
      toast.error(msg.includes("Cannot reach") ? msg : `Failed to open folder picker: ${msg}`)
    }
  }
  ...
  const handlePublishSelected = async (): Promise<void> => {
    ...
    try {
      ...
      for (const row of validRows) {
        if (!row.matchedShotId || !row.inputPath) continue
        ...
        if (row.publishTarget === "elements" || row.publishTarget === "both") {
          await addJob(row.inputPath, options, {
            tab: "element",
      ...
      const msg = err instanceof Error ? err.message : String(err)
      toast.error(`Failed to queue publish jobs: ${msg}`)
    } finally {
    ...
    <div className="flex flex-col h-full overflow-hidden bg-[#121212]">
      ...
      <div className="flex-1 overflow-auto p-4 md:p-6 space-y-4">
        <Card className="bg-[#1A1A1A] border border-[#404040]">
          <CardContent className="p-4 md:p-5 space-y-4">
            <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <Button
                  ...
                  disabled={!projectId || !engineReady || loading}
                ...
                <Button
                  ...
                  disabled={!projectId || !engineReady || !selectedPath || loading}
                  className="h-9 gap-2 border-[#404040] bg-[#121212] hover:bg-[#202020]"
                >
            ...
            {!engineReady && (
              <div className="rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs text-red-200 flex flex-wrap items-center gap-2">
                <AlertTriangle className="w-4 h-4 shrink-0" />
                <span>
                  {engineHealth.isChecking
                    ? `Checking engine at ${engineHealth.engineBase}…`
                    : engineHealth.offlineHelpText ??
                      `Bulk ingest needs the local engine at ${engineHealth.engineBase}. Start CTrack Engine from the system tray on this PC.`}
                </span>
              </div>
            )}
      ...
      <div className="flex-shrink-0 border-t border-[#404040] bg-[#1A1A1A] px-6 py-3 flex items-center justify-between gap-3">
        ...
        <Button
          ...
          disabled={selectedIds.size === 0 || rows.length === 0 || isPublishing || !engineReady}
        >
