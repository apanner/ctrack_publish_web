...
export function usePublishQueue() {
    ...
    const addJob = useCallback(async (filePath: string, options?: PublishJob['options'], meta?: PublishJobMeta, customContext?: { projectId?: string | null, projectCode?: string | null, episodeCode?: string | null, shotId?: string | null, shotCode?: string | null, sequenceName?: string | null, taskId?: string | null, taskName?: string | null, trackingNumber?: string | null }): Promise<string> => {
        ...
        const ipc = (window as any).ipcRenderer;
        try {
            await ipc.invoke('queue:add-job', {
                id: newJob.id,
            ...
            await ipc.invoke('queue:add-event', {
                job_id: newJob.id,
                run_id: null,
                attempt: 1,
                level: 'info',
            ...
            console.error('Failed to persist queued job', err);
    ...
    const startPublish = useCallback(async (jobId?: string) => {
        ...
        try {
            ...
            if (job.options?.gif !== false) {
                ...
                const isFrameSeq = !isInputVideoFile && resolvedFrameStart != null && resolvedFrameEnd != null;
                const transcodeInputPath = isFrameSeq
                    ? job.filePath.replace(/(\d+)(\.\w+)$/, (_, digits, ext) => `%0${digits.length}d${ext}`)
                    : job.filePath;
                addLog('info', `[TRANSCODE] Input: ${transcodeInputPath} | frame_start=${resolvedFrameStart ?? 'auto'} frame_end=${resolvedFrameEnd ?? 'auto'}`);
                const pythonResult = await (window as any).ipcRenderer.invoke('python-command', {
                    ...
                    params: {
                        ...
                        transcode_options: {
                            ...
                            start_frame: resolvedFrameStart ?? undefined,
                ...
                if (!transcodeResult || typeof transcodeResult !== "object") {
                    const top = pythonResult as { status?: string; message?: string };
                    throw new Error(
                        top?.message
                        ?? ((top as any)?.error)
                        ?? (top?.status === "error" ? "Transcode failed (no result)" : "Transcode returned no result — engine may have crashed (check engine terminal)")
                    );
                }
                ...
                const isFrameSeqAlt = !isInputVideoFile && resolvedFrameStart != null && resolvedFrameEnd != null;
                const transcodeInputPath = isFrameSeqAlt
                    ? job.filePath.replace(/(\d+)(\.\w+)$/, (_, digits, ext) => `%0${digits.length}d${ext}`)
                    : job.filePath;
    ...
    const startPublishElement = useCallback(async (jobId: string, options?: { elementNotes?: string }) => {
        ...
        try {
            ...
            if (isVideoFile || isImageSequence) {
                ...
                const transcodeInputPath = isImageSequence
                    ? job.filePath.replace(/(\d+)(\.\w+)$/, (_, digits, ext) => `%0${digits.length}d${ext}`)
                    : job.filePath;
                addLog('info', `[TRANSCODE] Input: ${transcodeInputPath} | frame_start=${resolvedFrameStart ?? 'auto'} frame_end=${resolvedFrameEnd ?? 'auto'}`);
                const pythonResult = await (window as any).ipcRenderer.invoke('python-command', {
