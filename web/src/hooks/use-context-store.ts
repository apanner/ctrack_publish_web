...
import type { ActiveStudio } from "@/lib/studio"
...
interface ContextState {
  ...
  setStudio: (id: string | null, studio: ActiveStudio | null) => void
...
export const useContextStore = create<ContextState>((set) => ({
  ...
  setStudio: (id, studio) =>
    set((state) => {
      if (state.studioId === id) {
        return { studioId: id, studio }
      }
      return {
        studioId: id,
        studio,
  ...
  setShotId: (id, code, sequence, epCode, epId) =>
    set({
      shotId: id,
      shotCode: code,
