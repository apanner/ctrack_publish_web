...
import type { BrowserSequenceFrameCheck } from "@/lib/delivery-media-scan"
...
interface SequenceHealthBarProps {
  ...
  dirPath: string | null
  /** When media was scanned in-browser, check gaps locally (no engine folder scan). */
  browserFrameCheck?: BrowserSequenceFrameCheck | null
  className?: string
}

export function SequenceHealthBar({ dirPath, browserFrameCheck, className }: SequenceHealthBarProps) {
  const [result, setResult] = useState<{ status: "ready" | "error"; missing: number[]; count: number; totalExpected: number } | null>(null)
  const [loading, setLoading] = useState(false)

  useEffect(() => {
    if (browserFrameCheck) {
      setLoading(false)
      const { frameStart, frameEnd, presentFrames } = browserFrameCheck
      const present = new Set(presentFrames)
      ...
      setResult({
        ...
        missing,
        count: presentFrames.length,
        totalExpected: frameEnd - frameStart + 1,
      })
      return
    }
  ...
  }, [dirPath, browserFrameCheck])

  if (!dirPath?.trim() && !browserFrameCheck) return null
  if (loading) {
    return (
