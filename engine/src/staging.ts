...
function processFileEntries(
  ...
  type SeqEntry = { frame: number; name: string; dirPath: string }
  ...
  for (const fp of filePaths) {
    ...
    if (IMAGE_EXTS.has(ext)) {
      ...
      if (m) {
        ...
        sequences.get(key)!.push({ frame, name, dirPath })
      } else {
        results.push({ filePath: fullPath, fileName: name, size: stat.size })
      }
      continue
  ...
  sequences.forEach((entries) => {
    ...
    const firstPath = path.join(firstEntry.dirPath, firstEntry.name)

    let totalSize = 0
    for (const entry of entries) {
      try {
        totalSize += fs.statSync(path.join(entry.dirPath, entry.name)).size
      } catch {
        /* skip */
      }
    }
