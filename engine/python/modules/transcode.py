...
def _is_exr_sequence_path(input_path: str) -> bool:
    """True if input looks like an OpenEXR image sequence pattern (not a flat video file)."""
    if not input_path:
        return False
    low = input_path.lower()
    if ".exr" not in low:
        return False
    return "%" in input_path or "#" in input_path


def _transcode_mode(options: Optional[dict]) -> str:
    if options and options.get("transcode_mode"):
        return str(options.get("transcode_mode")).strip().lower()
    env_mode = (os.environ.get("CTRACK_TRANSCODE_MODE") or "").strip().lower()
    if env_mode:
        return env_mode
    try:
        from modules.engine_settings import load_settings

        return str(load_settings().get("transcodeMode") or "auto").strip().lower()
    except Exception:
        return "auto"


def _exr_colorspace_mode(options: Optional[dict]) -> str:
    """Env CTRACK_EXR_COLORSPACE or options['exr_colorspace']: rec709_linear | rec2020_linear | none."""
    if options and options.get("exr_colorspace") is not None:
        return str(options.get("exr_colorspace") or "rec709_linear").strip().lower()
    return (os.environ.get("CTRACK_EXR_COLORSPACE") or "rec709_linear").strip().lower()

def _ffmpeg_escape_filter_path(path: str) -> str:
    """
    Escape a filesystem path for ffmpeg filter arguments.
    Handles Windows drive colon and backslashes, plus single quotes.
    """
    p = path.replace("\\", "/").replace(":", "\\:")
    return p.replace("'", r"\'")

def _resolve_ocio_lut_path(options: Optional[dict]) -> Optional[str]:
    """
    Optional OCIO display LUT path for exact-ish Nuke review matching.
    Priority: options['ocio_lut_path'] -> CTRACK_OCIO_LUT_PATH env.
    """
    raw = None
    if options and options.get("ocio_lut_path"):
        raw = str(options.get("ocio_lut_path"))
    elif os.environ.get("CTRACK_OCIO_LUT_PATH"):
        raw = os.environ.get("CTRACK_OCIO_LUT_PATH")
    if not raw:
        return None
...
def _build_exr_ocio_lut_filter(options: Optional[dict], input_path: str) -> Optional[str]:
    """
    If a LUT is configured and input is EXR sequence, apply lut3d.
    This is the closest way to match Nuke OCIO display transform in ffmpeg.
    """
    if not _is_exr_sequence_path(input_path):
        return None
    lut = _resolve_ocio_lut_path(options)
    if not lut:
        return None
    # trilinear gives stable speed/quality for review publishes.
    return "lut3d=file='{}':interp=trilinear".format(_ffmpeg_escape_filter_path(lut))


def _exr_display_vf_chain(options: Optional[dict]) -> Optional[str]:
    """
    Linear/scene-referred OpenEXR → display-referred Rec.709 (limited TV range) for yuv420p encoders.
    Uses zscale (libzimg). Matches typical Nuke/Resolve linear comp EXR → HEVC review encode expectations.
    Set exr_colorspace / CTRACK_EXR_COLORSPACE to 'none' to skip.
    """
    mode = _exr_colorspace_mode(options)
    if mode in ("none", "off", "false", "0", ""):
        return None
    # Input: planar float RGB from exr (matrixin=gbr); linear light, primaries tagged 709 or wide.
    # Output: bt709 transfer (gamma 2.4-ish tagging), limited range for MP4.
    presets = {
        "rec709_linear": (
            "zscale=transferin=linear:primariesin=bt709:matrixin=gbr:"
            "primaries=bt709:transfer=bt709:matrix=bt709:rangein=full:range=tv"
        ),
        "rec2020_linear": (
            "zscale=transferin=linear:primariesin=bt2020:matrixin=bt2020nc:"
            "primaries=bt709:transfer=bt709:matrix=bt709:rangein=full:range=tv"
        ),
        # Alias for wide / ACEScg-style working space approximated as BT.2020
        "wide_linear": (
            "zscale=transferin=linear:primariesin=bt2020:matrixin=bt2020nc:"
            "primaries=bt709:transfer=bt709:matrix=bt709:rangein=full:range=tv"
        ),
    }
    return presets.get(mode, presets["rec709_linear"])


def _prepend_exr_colorspace_filters(filters: list, input_path: str, options: Optional[dict]) -> None:
    ocio_lut_chain = _build_exr_ocio_lut_filter(options, input_path)
    if ocio_lut_chain:
        filters.insert(0, ocio_lut_chain)
        return
    chain = _exr_display_vf_chain(options) if _is_exr_sequence_path(input_path) else None
    if chain:
        filters.insert(0, chain)


def _append_output_bt709_color_tags(cmd: list) -> None:
    """Tag MP4 output as Rec.709 so players decode correctly."""
    cmd.extend(
        [
            "-colorspace",
            "bt709",
...
def _append_output_color_tags_for_exr(cmd: list, input_path: str, options: Optional[dict]) -> None:
    """
    Tag EXR-derived output with transfer characteristics matching active pipeline.
    - OCIO LUT path implies display-sRGB intent -> iec61966-2-1
    - zscale fallback uses bt709 transfer tags.
    """
    if not _is_exr_sequence_path(input_path):
        return
    if _build_exr_ocio_lut_filter(options, input_path):
        cmd.extend(
            [
                "-colorspace",
                "bt709",
        ...
        return
    _append_output_bt709_color_tags(cmd)


def _ensure_start_frame_alias(options):
    """Sync frame_start ↔ start_frame. Renderer sends frame_start; FFmpeg helpers use start_frame."""
    fs = options.get('frame_start')
    sf = options.get('start_frame')
    if sf is None and fs is not None:
        options['start_frame'] = fs
    elif fs is None and sf is not None:
        options['frame_start'] = sf


def _detect_actual_start_frame(input_path):
    """
    When frame_start is missing or 1 but the sequence pattern looks like 1001+, probe the disk.
    Replaces the %0Nd printf token with a glob wildcard, finds matching files, and returns the
    minimum frame number found.  Returns None if discovery fails.
    """
    m = re.search(r'%(\d*)d', input_path)
    if not m:
        return None
    glob_pattern = re.sub(r'%\d*d', '*', input_path)
    # On Windows forward/back slashes may differ; normalise.
    glob_pattern = glob_pattern.replace('\\', '/')
    matches = glob_module.glob(glob_pattern)
    if not matches:
        # Try again with OS-native separator
        glob_pattern2 = re.sub(r'%\d*d', '*', input_path)
        matches = glob_module.glob(glob_pattern2)
    if not matches:
        return None
    width_str = m.group(1)
    frame_re = re.sub(r'%\d*d', r'(\\d+)', re.escape(input_path))
    frames = []
    for f in matches:
        fm = re.match(frame_re, f.replace('\\', '/'))
        if not fm:
            fm = re.match(frame_re, f)
        if fm:
            try:
...
def transcode_chunk(input_path, output_path, options, log_callback=None):
    ...
    """
    opts = dict(options)
    _ensure_start_frame_alias(opts)
    ...
    filters = []
    _prepend_exr_colorspace_filters(filters, input_path, opts)
    if burnin_enabled:
        font = get_font_path()
        if font:
    ...
    _append_output_color_tags_for_exr(cmd, input_path, opts)
...
def transcode_to_mp4(input_path, output_path, options=None, log_callback=None):
    ...
    if _transcode_mode(options) in ("oiio", "nuke", "auto", "smart") and _is_exr_sequence_path(input_path):
        from modules.transcode_router import transcode_exr_smart

        return transcode_exr_smart(input_path, output_path, options, log_callback)
...
def transcode_sequence(input_path, output_path, options=None, log_callback=None):
    ...
    try:
        ...
        filters = []
        _prepend_exr_colorspace_filters(filters, input_path, options)
        if burnin_enabled:
            font = get_font_path()
            if font:
                shot_text = f"{metadata.get('shot', 'SHOT')} | {metadata.get('version', 'v001')}"
        ...
        # Input section — only input options before -i
        cmd = []
        frames_v: Optional[int] = None
        if '%' in input_path or '#' in input_path:
            start_num = options.get('start_frame')
            if start_num is None or start_num == 1:
                # If the caller didn't give us a reliable start frame, probe the disk.
                # This handles the common case where EXR sequences start at 1001 but
                # frame_start was not forwarded correctly from the browser.
                detected = _detect_actual_start_frame(input_path)
                if detected is not None and (start_num is None or detected != 1):
                    start_num = detected
                    options['start_frame'] = start_num
                    options['frame_start'] = start_num
                    if log_callback:
                        log_callback(f"Auto-detected start frame from disk: {start_num}")
                elif start_num is None:
                    start_num = 1
            cmd.extend(['-start_number', str(start_num)])
            cmd.extend(['-framerate', str(options.get('fps', 24))])
            frame_end = options.get('frame_end')
            if frame_end is not None:
                try:
                    nfr = int(frame_end) - int(start_num) + 1
                    if nfr > 0:
                        frames_v = nfr  # applied as output option after -i
        ...
        if frames_v is not None:
            cmd.extend(['-frames:v', str(frames_v)])
        ...
        _append_output_color_tags_for_exr(cmd, input_path, options)
        cmd.append(output_path)
...
def generate_preview_webp(input_path, output_path, options=None, log_callback=None):
    ...
    try:
        ...
        if is_sequence:
            sf = opts.get('start_frame')
            if sf is None or sf == 1:
                detected = _detect_actual_start_frame(input_path)
                if detected is not None and (sf is None or detected != 1):
                    sf = detected
                    if log_callback:
                        log_callback(f"WebP: auto-detected start frame from disk: {sf}")
            if sf is None:
                sf = 1
            cmd.extend(['-start_number', str(sf)])
        else:
