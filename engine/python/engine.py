...
def check_dependencies():
    ...
    oiio_path = get_oiiotool_path()
    if oiio_path == 'oiiotool' or not os.path.isfile(oiio_path):
        missing.append('oiiotool (run npm install to provision engine/runtime)')
    if not get_ocio_config_path():
        missing.append('OCIO config (engine/runtime/ocio)')
...
def main():
    while True:
        try:
            ...
            if command == 'ping':
            ...
            elif command == 'get_engine_status':
                from modules.runtime_probe import probe_runtime
                result = {'status': 'success', 'data': probe_runtime()}

            elif command == 'rescan_tools':
                from modules.engine_settings import save_settings, set_nuke_installations
                from modules.nuke_detect import detect_nuke_installations, resolve_sample_nk_template
                installs = detect_nuke_installations()
                settings = set_nuke_installations(installs)
                ...
                if results['transcode']['status'] == 'error':
                    tc = results['transcode']
                    result = {
                        'status': 'error',
                        'message': tc.get('message', 'Transcode failed'),
                        'transcode': tc,
                        'webp': results.get('webp'),
                    }
                else:
